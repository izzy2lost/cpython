...
#ifdef MS_WINDOWS
#if Py_WIN8APP
    /* XXX check for environment variables in apps */
#else
...
{
#if Py_WIN8APP
    /* Changing WD is not supported in metro app. */
...
#if !Py_WIN8APP
    /* XXX */
    PyErr_SetString(PyExc_NotImplementedError, "access");
    return NULL;
#endif
...
#if Py_WIN8APP
    /* XXX */
    PyErr_SetString(PyExc_NotImplementedError, "chmod");
    return NULL;
#else
    DWORD attr;
#endif
...
#if Py_WIN8APP
        wchar_t *p;
        /* AppCert says that GetCurrentDirectoryW is banned, even though it's 
           in the header file and can be called. Return app directory for now, computed from the executable name. */
...
#if Py_WIN8APP
    PyErr_SetString(PyExc_TypeError, "bytes API not supported in App");
    return NULL;
#else
...
#if Py_WIN8APP
    {
        PyErr_SetString(PyExc_NotImplementedError, "mkdir with bytes");
        goto exit;
    }
#else
...
#if Py_WIN8APP
    /* XXX MoveFile on WinRT */
    PyErr_SetString(PyExc_NotImplementedError,
                    "rename not supported on WinRT");
    return NULL;
#else
    memset(&src, 0, sizeof(src));
    memset(&dst, 0, sizeof(dst));
...
#if Py_WIN8APP
    {
        PyErr_SetString(PyExc_NotImplementedError, "rmdir with bytes");
        goto exit;
    }
#else
    ...
    if (GetFileAttributesExW(lpFileName, GetFileExInfoStandard, &info)) {
        ...
        if(is_directory &&
           info.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) {
            find_data_handle = FindFirstFileExW(lpFileName, FindExInfoStandard, 
                &find_data, FindExSearchNameMatch, NULL, 0);
...
#if Py_WIN8APP
    {
        PyErr_SetString(PyExc_NotImplementedError, "rm with bytes");
        goto exit;
    }
#else
...
#if Py_WIN8APP
    /* XXX */
    PyErr_SetString(PyExc_NotImplementedError, "utime");
    return NULL;
#endif
    memset(&path, 0, sizeof(path));
...
#if Py_WIN8APP
    PyErr_SetString(PyExc_NotImplementedError, "no process termination in metro app");
    return NULL;
#else
...
#if !defined(HAVE_READLINK) && defined(MS_WINDOWS) && !Py_WIN8APP
...
{
#if Py_WIN8APP
    PyErr_SetString(PyExc_NotImplementedError, "no process execution times in Metro app");
    return NULL;
#else
...
#if Py_WIN8APP
	/* AppCert says CreatePipe is banned */
	PyErr_SetString(PyExc_NotImplementedError, "pipe");
	return NULL;
#elif defined(MS_WINDOWS)
...
#if Py_WIN8APP
    PyObject *os1;
    PyErr_SetString(PyExc_ValueError, "no environment variables in a Metro app");
    return NULL;
#else
    PyObject *os1, *os2;
    wchar_t *newenv;
...
#ifdef MS_WINDOWS
#if Py_WIN8APP
	PyErr_SetString(PyExc_NotImplementedError, "cannot get CPU count on Windows Runtime");
	return NULL;
#else
    SYSTEM_INFO sysinfo;
...
#if Py_WIN8APP
	PyErr_SetString(PyExc_NotImplementedError, "no subprocesses on this system");
	return NULL;
#else
    if (inheritable)
