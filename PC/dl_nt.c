...
WCHAR dllVersionBuffer[16] = L""; // a private buffer

// Python Globals
HMODULE PyWin_DLLhModule = NULL;
const WCHAR *PyWin_DLLVersionString = dllVersionBuffer;
...
ULONG_PTR _Py_ActivateActCtx()
{
    ...
    if (PyWin_DLLhActivationContext && pfnActivateActCtx)
        if (!(*pfnActivateActCtx)(PyWin_DLLhActivationContext, &ret)) {
            OutputDebugStringW(L"Python failed to activate the activation context before loading a DLL\n");
...
void _Py_DeactivateActCtx(ULONG_PTR cookie)
{
    if (cookie && pfnDeactivateActCtx)
        if (!(*pfnDeactivateActCtx)(0, cookie))
            OutputDebugStringW(L"Python failed to de-activate the activation context\n");
}
#endif /* HAVE_SXS */
                                                ...
                                                LPVOID lpReserved)
{
    switch (ul_reason_for_call)
    {
        case DLL_PROCESS_ATTACH:
            ...
            /* XXX win8app */
            LoadStringW(hInst, 1000, dllVersionBuffer, sizeof(dllVersionBuffer));
#endif
            ...
            if (pfnGetCurrentActCtx && pfnAddRefActCtx)
                if ((*pfnGetCurrentActCtx)(&PyWin_DLLhActivationContext))
                    if (!(*pfnAddRefActCtx)(PyWin_DLLhActivationContext))
                        OutputDebugStringW(L"Python failed to load the default activation context\n");
#endif
            break;
