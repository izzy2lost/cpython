...
UDPsocket SDLNet_UDP_Open(Uint16 port)
{
  ...
  */
  /* FIXME: Add support for joining arbitrary groups to the API */
  {
    ...
    setsockopt(sock->channel, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&g, sizeof(g));
  }
#endif
...
IPaddress *SDLNet_UDP_GetPeerAddress(UDPsocket sock, int channel)
{
  ...
  switch (channel)
  {
    ...
    default:
      ...
      if (ValidChannel(channel) && (sock->binding[channel].numbound > 0))
      {
...
int SDLNet_UDP_SendV(UDPsocket sock, UDPpacket **packets, int npackets)
{
  ...
  for (i = 0; i < npackets; ++i)
  {
    ...
    if (packets[i]->channel < 0)
    {
      ...
      sock_addr.sin_family      = AF_INET;
      status = sendto(sock->channel, packets[i]->data, packets[i]->len, 0, (struct sockaddr *)&sock_addr, sock_len);
      if (status >= 0)
      {
...
extern int SDLNet_UDP_RecvV(UDPsocket sock, UDPpacket **packets)
{
  ...
  while (packets[numrecv] && SocketReady(sock->channel))
  {
    ...
    sock_len       = sizeof(sock_addr);
    packet->status = recvfrom(sock->channel, packet->data, packet->maxlen, 0, (struct sockaddr *)&sock_addr, &sock_len);
    if (packet->status >= 0)
    {
      ...
      for (i = (SDLNET_MAX_UDPCHANNELS - 1); i >= 0; --i)
      {
        ...
        for (j = binding->numbound - 1; j >= 0; --j)
        {
          if ((packet->address.host == binding->address[j].host) && (packet->address.port == binding->address[j].port))
          {
            packet->channel = i;
